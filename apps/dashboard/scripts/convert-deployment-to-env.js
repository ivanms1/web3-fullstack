...
function convertDeploymentToEnv() {
    try {
        ...
        const deploymentFiles = files.filter(file =>
            file.startsWith('deployment-info') && file.endsWith('.json')
        );

        if (deploymentFiles.length === 0) {
            throw new Error('No deployment info files found in scripts/ directory. Please place your deployment-info*.json file in the scripts/ folder.');
        }
...
`;
        ...
        console.log('3. The app will now use environment variables instead of JSON files');
